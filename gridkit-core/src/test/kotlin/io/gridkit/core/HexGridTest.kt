...
class HexGridTest {
    ...
    @Test fun `even-row TOP_LEFT  → (row-1, col-1)`() =
        assertEquals(HexCoordinate(1, 1), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.TOP_LEFT))

    @Test fun `even-row TOP_RIGHT → (row-1, col)`() =
        assertEquals(HexCoordinate(1, 2), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.TOP_RIGHT))

    @Test fun `even-row LEFT      → (row, col-1)`() =
        assertEquals(HexCoordinate(2, 1), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.LEFT))

    @Test fun `even-row RIGHT     → (row, col+1)`() =
        assertEquals(HexCoordinate(2, 3), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.RIGHT))

    @Test fun `even-row DOWN_LEFT  → (row+1, col-1)`() =
        assertEquals(HexCoordinate(3, 1), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.DOWN_LEFT))

    @Test fun `even-row DOWN_RIGHT → (row+1, col)`() =
        assertEquals(HexCoordinate(3, 2), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(2, 2), HexEdgeDirection.DOWN_RIGHT))

    // ── Odd-row neighbor offsets ──────────────────────────────────────────────

    @Test fun `odd-row TOP_LEFT  → (row-1, col)`() =
        assertEquals(HexCoordinate(0, 2), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.TOP_LEFT))

    @Test fun `odd-row TOP_RIGHT → (row-1, col+1)`() =
        assertEquals(HexCoordinate(0, 3), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.TOP_RIGHT))

    @Test fun `odd-row LEFT      → (row, col-1)`() =
        assertEquals(HexCoordinate(1, 1), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.LEFT))

    @Test fun `odd-row RIGHT     → (row, col+1)`() =
        assertEquals(HexCoordinate(1, 3), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.RIGHT))

    @Test fun `odd-row DOWN_LEFT  → (row+1, col)`() =
        assertEquals(HexCoordinate(2, 2), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.DOWN_LEFT))

    @Test fun `odd-row DOWN_RIGHT → (row+1, col+1)`() =
        assertEquals(HexCoordinate(2, 3), HexGrid<Nothing>(5, 5).getNeighbor(HexCoordinate(1, 2), HexEdgeDirection.DOWN_RIGHT))

    @Test
    fun `getNeighbor returns null at boundary`() {
        val grid = HexGrid<Nothing>(5, 5)
        assertNull(grid.getNeighbor(HexCoordinate(0, 0), HexEdgeDirection.TOP_LEFT))
        assertNull(grid.getNeighbor(HexCoordinate(0, 0), HexEdgeDirection.TOP_RIGHT))
    }
    ...
    fun `placeNext even-row RIGHT creates (0,1)`() {
        ...
        val cell = grid.placeNext(HexCoordinate(0, 0), HexEdgeDirection.RIGHT)
    ...
    fun `placeNext is idempotent`() {
        ...
        val a = grid.placeNext(HexCoordinate(1, 1), HexEdgeDirection.RIGHT)
        val b = grid.placeNext(HexCoordinate(1, 1), HexEdgeDirection.RIGHT)
        assertTrue(a === b)
    }
    ...
    fun `placeNext expands bounding box`() {
        val grid = HexGrid<Nothing>(1, 1)
        grid.placeNext(HexCoordinate(0, 0), HexEdgeDirection.DOWN_RIGHT)
    ...
    fun `cell carries typed data payload`() {
        val grid = hexGrid<Int>(4, 4) { place(HexCoordinate(1, 1), data = 42) }
    ...
    fun `getDirectedNeighbors returns all 6 directions for interior cell`() {
        ...
        assertTrue(HexEdgeDirection.LEFT in map)
        assertTrue(HexEdgeDirection.RIGHT in map)
    }
}
